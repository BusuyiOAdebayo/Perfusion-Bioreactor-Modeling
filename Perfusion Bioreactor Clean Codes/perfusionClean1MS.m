...
Theta = zeros(noOfIntervals,15);%zeros(0,noOfIntervals);

theta0 = zeros(noOfIntervals,15);
theta0(:,1) = 10; % mumax
theta0(:,2) = 10; % Ksglc
theta0(:,3) = 10; % Ksglu
theta0(:,4) = 10; % Kilac
...
%[theta,Rsdnrm,Rsd,ExFlg,OptmInfo,Lmda,Jmat] = lsqcurvefit(@perfusionBioreactorSimulator,theta0,t,c);
loopCounter = 0;
for i = 1:noOfIntervals
    %     tt = [t_exp(i:i+1);0.95*t_exp(i:i+1)];
    %     cc = [c_exp(i:i+1,:);0.9*c_exp(i:i+1,:)];
    %theta = zeros(noOfIntervals,1);
    %[theta,Rsdnrm,Rsd,ExFlg,OptmInfo,Lmda,Jmat] = lsqcurvefit(@(theta,t)perfusionBioreactorSimulator(theta,t,Fbleed,Xv_target,cfeed,V,CSPR,PR,proportioninHarvest),theta0(i,:),t_exp,c_exp,LB,UB,optionsOpt);
    %theta = lsqcurvefit(@(theta,t)perfusionBioreactorSimulator(theta,t,Fbleed(i),Xv_target,cfeed,V,CSPR,PR,proportioninHarvest,cinit,optionsSim),theta0(i,:),t_exp,c_exp,LB,UB,optionsOpt); %t_exp(i:i+1),c_exp(i:i+1,:)
    %theta = lsqcurvefit(@(theta,t)perfusionBioreactorSimulator(theta,t,Fbleed(i),Xv_target,cfeed,V,CSPR,PR,proportioninHarvest,cinit,optionsSim),theta0(i,:),t_exp(i:i+1),c_exp(i:i+1,:),LB,UB,optionsOpt); %t_exp(i:i+1),c_exp(i:i+1,:), LB(i,:),UB(i,:)
    theta = lsqcurvefit(@(theta,t)perfusionBioreactorSimulator(theta,t,Fbleed(i),Xv_target,cfeed,V,CSPR,PR,proportioninHarvest,cinit,optionsSim),theta0(i,:),T_exp(2*i-1:2*i+1),C_exp(2*i-1:2*i+1,:),LB,UB,optionsOpt); %t_exp(i:i+1),c_exp(i:i+1,:), LB(i,:),UB(i,:)
    %     theta = [Theta,theta];
    %     Theta =
...
% Using anonymous/unnamed function
...
%for i = 1:nofusionBioreactor(t,c,theta,Fbleed,Xv_target,cfeed,V,CSPR,PR,proportioninHarvest),t,cinit,optionsSim,theta);
...
function objfun = costFunction(theta,C_exp,M,k)
U_P_Pred = opt_Var(1:nu*P);
U_P_Pred = reshape(U_P_Pred,nu,P);
X_P_Pred = opt_Var(nu*P+1:end);
X_P_Pred = reshape(X_P_Pred,nx,1+P);
objfun = (C_exp(:,j+1) - c(:,j+1));
end

function [cineq, ceq] = constraintsFunction(opt_Var,C_exp,k,odefunhandle)
U_P_Pred = opt_Var(1:nu*P);
U_P_Pred = reshape(U_P_Pred,nu,P);
X_P_Pred = opt_Var(nu*P+1:end);
X_P_Pred = reshape(X_P_Pred,nx,1+P);

lb_U = lb_opt_Var(1:nu*P);
lb_U = reshape(lb_U,nu,P);
ub_U = ub_opt_Var(1:nu*P);
ub_U = reshape(ub_U,nu,P);

lb_X = lb_opt_Var(nu*P+1:end);
lb_X = reshape(lb_X,nx,1+P);
ub_X = ub_opt_Var(nu*P+1:end);
ub_X = reshape(ub_X,nx,1+P);
X_P_Pred_ODESolver = zeros(nx,1+P);%X_P_Pred;%zeros(nx,1+P);
X0 = X_P_Pred(:,1);%X_P_Pred_ODESolver(:,1);%X_P_Pred(:,1);%[0;0;0]; % Initial concentrations
X_P_Pred_ODESolver(:,1) = X0;%x0;%X_P_Pred(:,1);

cineqx = zeros(2*nx,1+P);%zeros((nx-1)*(1+P),1);
cineqx(:,1) = [lb_X(:,1)-X_P_Pred(:,1);X_P_Pred(:,1)-ub_X(:,1)];
cinequ = zeros(2*nu,P);%zeros(nu*P,1);
cineq = [reshape(cineqx,[],1);reshape(cinequ,[],1)];

ceqx = zeros(nx,1+P);%zeros(nu*P,1);
ceqx(:,1) = X_P_Pred_ODESolver(:,1)-X_P_Pred(:,1);

for j = 1:P %[for X, for U and for dU]
    tSpanobjfun = [tVector(k), tVector(k)+deltat];
    [~,X_Pred] = ode45(odefunhandle,tSpanobjfun,X0,optionsSim,U_P_Pred(:,j));
    X_P_Pred_ODESolver(:,j+1) = X_Pred(end,:)';
    X0 = X_Pred(end,:)';
    cineqx(:,j+1) = [lb_X(:,j+1)-X_P_Pred(:,j+1);X_P_Pred(:,j+1)-ub_X(:,j+1)];
    cinequ(:,j) = [lb_U(:,j)-U_P_Pred(:,j);U_P_Pred(:,j)-ub_U(:,j)];

    ceqx(:,j+1) = X_P_Pred_ODESolver(:,j+1)-X_P_Pred(:,j+1);
    if j <= M
        cequ = [];
    else
        cequ = zeros(nu,P);%zeros(nu*P,1);
        cequ(:,j) = U_P_Pred(:,j)-U_P_Pred(:,j-1);
    end
end
ceq = [reshape(ceqx,[],1);reshape(cequ,[],1)];
end